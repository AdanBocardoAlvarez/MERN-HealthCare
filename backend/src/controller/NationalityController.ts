...
export class NationalityController {
    ...
    static async getAllService(req, res) {
        ...
        try {
            ...
            const serviceCount = await NationalityModel.find({ deleted_at: null }).count();
            ...
            let getAllService;
            if (!search_title) {
                getAllService = await NationalityModel.find({ deleted_at: null })
                    .skip((perPage * page) - perPage).limit(perPage);
            }
            else {
                getAllService = await NationalityModel.find({ deleted_at: null, name: { $regex: search_title, $options: 'i' } })
                    .skip((perPage * page) - perPage).limit(perPage);
            }
    ...
    static async storeService(req, res) {
        try {
            await NationalityModel.create({ name: req.body.name, created_at: new Date() })
    ...
    static async getRecordById(req, res) {
        ...
        try {
            ...
            const updatedPost = await NationalityModel.findOne({ _id: id });
            if (updatedPost) {
    ...
    static async editService(req, res) {
        ...
        try {
            await NationalityModel.updateOne({ _id: data_id }, {
                name: titleName,
                updated_at: new Date()
            });
    ...
    static async getDataWithTrash(req, res) {
        ...
        try {
            const serviceCount = await NationalityModel.find({ deleted_at: { $ne: null } }).count();
            totalPages = Math.ceil(serviceCount / perPage);
            if (totalPages === page || totalPages === 0) {
                pageToken = null;
            }
            ...
            let getAllService;
            if (!search_title) {
                getAllService = await NationalityModel.find({ deleted_at: { $ne: null } })
                    .skip((perPage * page) - perPage).limit(perPage);
            }
            else {
                getAllService = await NationalityModel.find({ deleted_at: { $ne: null }, name: { $regex: search_title, $options: 'i' } })
                    .skip((perPage * page) - perPage).limit(perPage);
            }
    ...
    static async tempDeleteService(req, res) {
        try {
            await NationalityModel.updateOne({ _id: req.query.id }, { deleted_at: new Date() });
    ...
    static async restoreService(req, res) {
        try {
            await NationalityModel.updateOne({ _id: req.body.id }, { deleted_at: null });
    ...
    static async deleteService(req, res) {
        try {
            await NationalityModel.findByIdAndDelete({ _id: req.query.id });
    ...
    static async tempMultiDelteService(req, res) {
        ...
        try {
            await NationalityModel.updateMany({ '_id': { '$in': arr } },
    ...
    static async multiDelteService(req, res) {
        ...
        let arr: Array<string> = str.split(",");
        try {
            await NationalityModel.deleteMany({ '_id': { $in: arr } }, { deleted_at: { $ne: null } });
            res.json({ 'status': 200, 'message': 'Record Permanently Delete' });
        } catch (error) {
            res.json({ 'status': false, 'message': error.message });
        }
    ...
    static async activeService(req, res) {
        try {
            await NationalityModel.updateOne({ _id: req.body.id }, { status: req.body.status });
