import React, { useEffect, useState } from 'react';
import { ColumnProps } from 'antd/es/table';
import { Button, Checkbox, Modal, Switch, Table } from 'antd';
import { IQuote } from '../../../../interfaces/Admin/quote';
import { useNavigate } from 'react-router-dom';
import { AdminApi } from '../../../../api/api';
import { openNotificationWithIcon } from '../../../components/Toast';
import { useSelector } from 'react-redux';
import { AppState } from '../../../../redux/store';
import { DeleteOutlined } from '@ant-design/icons';

type Props = {
	quote?: IQuote[];
};

const QuoteTable = ({ quote }: Props) => {
	const token = useSelector((state: AppState) => state.admin.Token);
	const [visibleModal, setVisibleModal] = useState();
	const [multipleSelect, setMultipleSelect] = useState([]);
	const [quotes, setQuotes] = useState([]);
	const [visibleModalMulti, setVisibleModalMulti] = useState([]);
	const iddd = quote.map((item) => item._id);
	const [multiCheckFlag, setMultiCheckFlag] = useState(false);
	const [checked, setChecked] = useState(true);
	const [isFirstRender, setIsFirstRender] = useState(true);
	const [status, setStatus] = useState({
		status: 1,
		id: ''
	});

	useEffect(() => {
		setQuotes(quote);
	}, [quote]);

	const handleCancel = () => {
		setVisibleModal(undefined);
	};

	const handleOk = () => {
		setVisibleModal(undefined);
	};

	const handleCancel3 = () => {
		setVisibleModalMulti([]);
	};

	const Navigate = useNavigate();

	function editquote(id, quote_title) {
		var url =
			'/admin/edit-quote/' + id + '/' + quote_title.author_name + '/' + quote_title.quote_title;
		Navigate(url);
	}

	function deleteQuote(id: string) {
		AdminApi.tempDeleteBlog(`quote/temp-delete?id=${id}`, token)
			.then((datas) => {
				const message = datas.message;
				const status = datas.status;
				if (status) {
					setVisibleModal(undefined);
					openNotificationWithIcon({ type: 'success', message });
					setQuotes((prevQuotes) => prevQuotes.filter((quotes) => quotes._id !== id));
				} else {
					openNotificationWithIcon({ type: 'error', message: message });
				}
			})
			.catch((err) => {
				const message = err?.response?.data?.message || err.message;
				openNotificationWithIcon({ type: 'error', message: message });
			});
	}

	function deleteMultiQuote(id: string[]) {
		AdminApi.tempDeleteBlog(`quote/temp-multi-delete?id=${id}`, token)
			.then((datas) => {
				const message = datas.message;
				const status = datas.status;
				if (status) {
					setVisibleModalMulti([]);
					openNotificationWithIcon({ type: 'success', message });
					setQuotes((prevQuotes) => prevQuotes.filter((quotes) => !id.includes(quotes._id)));
					setMultipleSelect([]);
				} else {
					openNotificationWithIcon({ type: 'error', message: message });
				}
			})
			.catch((err) => {
				const message = err?.response?.data?.message || err.message;
				openNotificationWithIcon({ type: 'error', message: message });
			});
	}

	function selectQuote(id, flag) {
		if (JSON.stringify(multipleSelect) === JSON.stringify(id) && flag === 'all') {
			multipleSelect.length = 0;
			setMultiCheckFlag(false);
		} else if (flag === 'multi') {
			setMultiCheckFlag(false);
			setMultipleSelect((prev) => {
				if (prev.includes(id)) {
					return prev.filter((item) => item !== id);
				} else {
					return [...prev, id];
				}
			});
		} else if (multipleSelect.length === 0) {
			setMultiCheckFlag(true);
			setMultipleSelect(id);
		} else {
			setMultiCheckFlag(true);
			setMultipleSelect(id);
		}
	}

	function setNavigate(nav) {
		Navigate(`${nav}`);
	}

	const activeBlog = (check: boolean, id) => {
		AdminApi.getQuotesList('quote/index', token)
			.then((datas) => {
				const result = datas.find((item) => item._id === id);
				if (result) {
					const statu = result.status;
					setChecked(check);
					setStatus({
						status: statu === 0 ? 1 : 0,
						id: id
					});
				}
			})
			.catch((err) => {
				const message = err?.response?.data?.message || err.message;
				openNotificationWithIcon({ type: 'error', message: message });
			});
	};

	useEffect(() => {
		if (!isFirstRender && status.id) {
			const urlSearchParams = new URLSearchParams();
			for (const key in status) {
				urlSearchParams.append(key, status[key]);
			}
			const formData = urlSearchParams.toString();
			AdminApi.createPost(formData, 'quote/status', token).then((datas) => {
				const message = datas.message;
				const status = datas.message;
				if (status) {
					openNotificationWithIcon({ type: 'success', message });
				} else {
					openNotificationWithIcon({ type: 'error', message: message });
				}
			});
		} else {
			setIsFirstRender(false);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [checked, status, token]);

	const columns: ColumnProps<IQuote>[] = [
		// {
		//   key: '_id',
		//   dataIndex: '_id',
		//   title: 'Select',
		//   render: (_id) => {
		//     return <Checkbox onChange={() => selectQuote(_id)} />;
		//   }
		// },

		{
			key: '_id',
			dataIndex: '_id',
			title: <Checkbox onChange={() => selectQuote(iddd, 'all')} checked={multiCheckFlag} />,
			render: (_id) => {
				return (
					<Checkbox
						onChange={() => selectQuote(_id, 'multi')}
						checked={
							multiCheckFlag
								? multiCheckFlag
								: multipleSelect.find((item) => (item === _id ? true : false))
						}
					/>
				);
			}
		},

		{
			key: '_id',
			dataIndex: '_id',
			title: 'S.No.',
			render: (_id) => {
				const index = quote.findIndex((item) => item._id === _id);
				return <strong>{index + 1}</strong>;
			}
		},
		{
			key: '_id',
			dataIndex: 'quote_title',
			title: 'Quote',
			render: (quote_title) => {
				return <strong>{quote_title}</strong>;
			}
		},
		{
			key: '_id',
			dataIndex: 'author_name',
			title: ' Author Name',
			render: (author_name) => {
				return <strong>{author_name}</strong>;
			}
		},
		{
			key: '_id',
			dataIndex: '_id',
			title: 'Actions',
			render: (_id, quote_title) => (
				<div className='buttons-list nowrap'>
					<Button shape='round' type='primary' onClick={() => editquote(_id, quote_title)}>
						Edit
					</Button>
					<Button shape='round' type='primary' danger onClick={() => setVisibleModal(_id)}>
						<DeleteOutlined /> Move to Bin
					</Button>
				</div>
			)
		},
		{
			key: '_id',
			dataIndex: '_id',
			title: 'Status',
			render: (_id, quote_title) => {
				return (
					<div className='buttons-list nowrap'>
						<Switch
							defaultChecked={quote_title.status === 0 ? false : true}
							onChange={() => activeBlog(!checked, _id)}
						/>
					</div>
				);
			}
		}
	];

	return (
		<>
			<div className='d-flex justify-content-end align-items-center'>
				{multipleSelect.length > 0 && (
					<Button
						className='mb-3'
						shape='round'
						type='primary'
						danger
						onClick={() => setVisibleModalMulti(multipleSelect)}
					>
						Delete Multiple
					</Button>
				)}
				<Button
					className='mb-3 ms-1 mx-2'
					shape='round'
					type='primary'
					danger
					onClick={() => setNavigate('/admin/view-trash-quote')}
				>
					<DeleteOutlined />
				</Button>
				<Button
					className='mb-3 ms-1 mx-2'
					shape='round'
					type='primary'
					onClick={() => setNavigate('/admin/add-quote')}
				>
					Add Quote
				</Button>
			</div>
			<Table
				// pagination={pagination}
				className='accent-header'
				rowKey='_id'
				dataSource={quotes}
				columns={columns}
			/>

			<Modal
				open={visibleModal}
				footer={null}
				onCancel={handleCancel}
				onOk={handleOk}
				className='d-flex '
				title={<h3 className='title text-center'>Delete Quote</h3>}
			>
				<Button shape='round' type='primary' onClick={() => deleteQuote(visibleModal)}>
					Move to Bin
				</Button>
				<Button shape='round' type='primary' onClick={() => setVisibleModal(undefined)}>
					Cancel
				</Button>
			</Modal>

			<Modal
				open={visibleModalMulti.length > 0}
				footer={null}
				onCancel={handleCancel3}
				className='d-flex '
				title={<h3 className='title text-center'>Delete Quote</h3>}
			>
				<Button
					shape='round'
					type='primary'
					onClick={() => deleteMultiQuote(visibleModalMulti)}
					danger
				>
					Temporary Multiple Delete
				</Button>
				<Button shape='round' type='primary' onClick={() => setVisibleModal(undefined)}>
					Cancel
				</Button>
			</Modal>
		</>
	);
};

export default QuoteTable;
