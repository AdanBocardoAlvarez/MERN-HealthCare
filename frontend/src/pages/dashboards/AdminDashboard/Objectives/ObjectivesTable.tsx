...
type Props = {
  viewObjectives?: IAdminPanel[];
};

const ObjectivesTable = ({ viewObjectives }: Props) => {
  const token = useSelector((state: AppState) => state.admin.Token);
  const [isFirstRender, setIsFirstRender] = useState(true);
  const [visibleModal, setVisibleModal] = useState();
  const [multipleSelect, setMultipleSelect] = useState([]);
  const [objectives, setObjectives] = useState([]);
  const iddd = viewObjectives.map((item) => item._id);
  ...
  function editObjectives(id, title) {
    var url = `/admin/edit-objectives/${id}/${title.name}`;
    Navigate(url);
  }

  function deleteTempObjectives(id: string) {
    AdminApi.tempDeleteBlog(`objectives/temp-delete?id=${id}`, token)
      .then((datas) => {
  ...
  function deleteMultiObjectives(id: string[]) {
    AdminApi.tempDeleteBlog(`objectives/temp-multi-delete?id=${id}`, token)
      .then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
  ...
  const activeObjectives = (check: boolean, id) => {
    AdminApi.getComman('objectives/index', token)
      .then((datas) => {
  ...
  useEffect(() => {
    if (!isFirstRender && status.id) {
      ...
      AdminApi.createPost(formData, 'objectives/status', token).then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
          openNotificationWithIcon({ type: 'success', message });
  ...
  const columns: ColumnProps<IAdminPanel>[] = [
...

    {
      ...
      render: (_id, title) => (
        <div className='buttons-list nowrap'>
          <Button shape='round' type='primary' onClick={() => editObjectives(_id, title)}>
            Edit
          </Button>
...

    {
      ...
      dataIndex: '_id',
      title: 'Status',
      render: (_id, Objectives) => {
        return (
          <div className='buttons-list nowrap'>
            <Switch
              defaultChecked={Objectives.status === 0 ? false : true}
              onChange={() => activeObjectives(!checked, _id)}
            />
          </div>
        );
      }
    ...
    <>
      <div className='d-flex justify-content-end align-items-center'>
        ...
        <Button
          ...
          type='primary'
          danger
          onClick={() => setNavigate('/admin/view-trash-objectives')}
        >
          <DeleteOutlined />
        </Button>
        <Button
          ...
          shape='round'
          type='primary'
          onClick={() => setNavigate('/admin/add-objectives')}
        >
          Add Objectives
        </Button>
      </div>
      ...
      >
        <Button
          shape='round'
          type='primary'
          onClick={() => deleteTempObjectives(visibleModal)}
          danger
        >
          Move to Bin
        </Button>
