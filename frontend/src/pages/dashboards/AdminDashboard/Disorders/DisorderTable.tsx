...
type Props = {
  viewDisorder?: IAdminPanel[];
};

const DisorderTable = ({ viewDisorder }: Props) => {
  const token = useSelector((state: AppState) => state.admin.Token);
  const [isFirstRender, setIsFirstRender] = useState(true);
  const [visibleModal, setVisibleModal] = useState();
  const [multipleSelect, setMultipleSelect] = useState([]);
  const [nationality, setNationality] = useState([]);
  const iddd = viewDisorder.map((item) => item._id);
  ...
  function deleteTempNationality(id: string) {
    AdminApi.tempDeleteBlog(`disorders/temp-delete?id=${id}`, token)
      .then((datas) => {
  ...
  function deleteMultiNationality(id: string[]) {
    AdminApi.tempDeleteBlog(`disorders/temp-multi-delete?id=${id}`, token)
      .then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
  ...
  const activeNationality = (check: boolean, id) => {
    AdminApi.getQuotesList('disorders/index', token)
      .then((datas) => {
  ...
  useEffect(() => {
    if (!isFirstRender && status.id) {
      ...
      AdminApi.createPost(formData, 'disorders/status', token).then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
          openNotificationWithIcon({ type: 'success', message });
      ...
      render: (_id, title) => (
        <div className='buttons-list nowrap'>
          <Button
            shape='round'
            type='primary'
            onClick={() => Navigate(`/admin/edit-disorders/${_id}/${title.name}`)}
          >
            <EditOutlined />
          </Button>
    ...
    <>
      <div className='d-flex justify-content-end align-items-center'>
        ...
        <Button
          ...
          type='primary'
          danger
          onClick={() => setNavigate('/admin/view-trash-disorders')}
        >
          <DeleteOutlined />
        </Button>
        <Button
          ...
          shape='round'
          type='primary'
          onClick={() => setNavigate('/admin/add-disorders')}
        >
          Add Disorder
        </Button>
      </div>
      ...
      <Modal
        ...
        className='d-flex '
        title={<h3 className='title text-center'>Delete Disorder</h3>}
      >
        <Button
          shape='round'
          type='primary'
