...
type Props = {
  keywordsTrash?: IAdminPanel[];
};

const KeywordsTrashTable = ({ keywordsTrash = [] }: Props) => {
  ...
  const iddd = keywordsTrash.map((item) => item._id);

  useEffect(() => {
    setKeywords(keywordsTrash);
  }, [keywordsTrash]);
  ...
  function restoreKeywords(id) {
    const urlSearchParams = new URLSearchParams();
    urlSearchParams.append('id', id);
    const formData = urlSearchParams.toString();

    AdminApi.RestorePatch(formData, 'keywords/restore', token)
      .then((datas) => {
  ...
  function deleteKeywords(id) {
    AdminApi.tempDeleteBlog(`keywords/delete?id=${id}`, token)
      .then((datas) => {
        const message = datas.message;
  ...
  function deleteMultiKeywords(id: string[]) {
    AdminApi.tempDeleteBlog(`keywords/multi-delete?id=${id}`, token)
      .then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
    ...
    <>
      <div className='d-flex justify-content-end align-items-center'>
        ...
        <Button
          ...
          shape='round'
          type='primary'
          onClick={() => setNavigate('/admin/view-keywords')}
        >
          View Table
        </Button>
      </div>
      ...
      >
        <Button shape='round' type='primary' onClick={() => deleteKeywords(visibleModal)}>
      ...
      <Modal
        ...
        title={<h3 className='title text-center'>Delete Keywords</h3>}
      >
        <Button shape='round' type='primary' onClick={() => deleteMultiKeywords(visibleModalMulti)}>
...
export default KeywordsTrashTable;
