...
const PrivacyTable = ({ ViewPrivacy }: Props) => {
  ...
  function deleteTempNationality(id: string) {
    AdminApi.tempDeleteBlog(`privacy-policy/temp-delete?id=${id}`, token)
      .then((datas) => {
  ...
  function deleteMultiNationality(id: string[]) {
    AdminApi.tempDeleteBlog(`privacy-policy/temp-multi-delete?id=${id}`, token)
      .then((datas) => {
        const message = datas.message;
        const status = datas.status;
        if (status) {
  ...
  const activeNationality = (check, id) => {
    ...
    formData.append('active_policy', check);
    AdminApi.createPost(formData, 'privacy-policy/status', token).then((datas) => {
      const message = datas.message;
      const status = datas.success;
      if (status) {
        openNotificationWithIcon({ type: 'success', message });
        AdminApi.getComman('privacy-policy/index', token)
          .then((datas) => {
    ...
    },
    {
      ...
      render: (_id, nationality) => {
        return (
          <div className='buttons-list nowrap'>
            <Switch
              checked={nationality.active_policy === 0 ? false : true}
              onChange={() => activeNationality(nationality.active_policy, _id)}
            />
          </div>
        );
      }
    ...
    <>
      <div className='d-flex justify-content-end align-items-center'>
        ...
        <Button
          ...
          type='primary'
          danger
          onClick={() => setNavigate('/admin/view-trash-privacy-policy')}
        >
          <DeleteOutlined />
        </Button>
        <Button
          ...
          shape='round'
          type='primary'
          onClick={() => setNavigate('/admin/add-privacy-policy')}
        >
          Add Privacy Policy
        </Button>
      </div>
      ...
      <Modal
        ...
        className='d-flex '
        title={<h3 className='title text-center'>Delete Privacy Policy</h3>}
      >
        <Button
          shape='round'
          type='primary'
