...
type Props = {
    viewCountry?: IAdminPanel[];
};

const CountryTable = ({ viewCountry }: Props) => {
    const token = useSelector((state: AppState) => state.admin.Token);
    const [isFirstRender, setIsFirstRender] = useState(true);
    const [visibleModal, setVisibleModal] = useState();
    const [multipleSelect, setMultipleSelect] = useState([]);
    const [country, setCountry] = useState([]);
    const iddd = viewCountry.map((item) => item._id);
    ...
    function deleteTempCountry(id: string) {
        AdminApi.tempDeleteBlog(`country/temp-delete?id=${id}`, token)
            .then((datas) => {
    ...
    function deleteMultiCountry(id: string[]) {
        AdminApi.tempDeleteBlog(`country/temp-multi-delete?id=${id}`, token)
            .then((datas) => {
                const message = datas.message;
                const status = datas.status;
                if (status) {
    ...
    const activeCountry = (check: boolean, id) => {
        AdminApi.getQuotesList('country/index', token)
            .then((datas) => {
    ...
    useEffect(() => {
        if (!isFirstRender && status.id) {
            ...
            AdminApi.createPost(formData, 'country/status', token).then((datas) => {
                const message = datas.message;
                const status = datas.status;
                if (status) {
                    openNotificationWithIcon({ type: 'success', message });
            ...
            render: (_id, title) => (
                <div className='buttons-list nowrap'>
                    <Button
                        shape='round'
                        type='primary'
                        onClick={() => Navigate(`/admin/edit-country/${_id}`)}
                    >
                        Edit
                    </Button>
        ...
        },
        {
            ...
            render: (_id, country) => {
                return (
                    <div className='buttons-list nowrap'>
                        <Switch
                            defaultChecked={country.status === 0 ? false : true}
                            onChange={() => activeCountry(!checked, _id)}
                        />
                    </div>
                );
            }
        ...
        <>
            <div className='d-flex justify-content-end align-items-center'>
                ...
                <Button
                    ...
                    type='primary'
                    danger
                    onClick={() => setNavigate('/admin/view-trash-country')}
                >
                    <DeleteOutlined />
                </Button>
                <Button
                    ...
                    shape='round'
                    type='primary'
                    onClick={() => setNavigate('/admin/add-country')}
                >
                    Add Country
                </Button>
            </div>
            ...
            >
                <Button
                    shape='round'
                    type='primary'
                    onClick={() => deleteTempCountry(visibleModal)}
                    danger
                >
                    Move to Bin
                </Button>
